...
class PodDroidApp(App, CommService):
    ...
    def build(self):
        ...
        CommService.__init__(self, 123, "localhost", 8000, printer=self.print_message)
    ...
    def setup_gui(self):
        ...
        ## Create label
        self.scroll_label = ScrollView(pos_hint={'center_x':0.3, 'center_y':0.3})
        ## Create label
        self.label = Label(text=constants.GUI_WELCOME_MSG, halign='left', size_hint_y= None, height="40dp")
        ...
        ## Create Textbox
        self.textbox = TextInput(size_hint_y=.1, size_hint_x=.8, multiline=False)
        self.textbox.focus = True
        self.textbox.bind(on_text_validate=self.handle_input)

        ## Create button
        self.enter_button = Button(text='Enter', size_hint_y=.1, size_hint_x=.2) 
        self.enter_button.bind(on_press=self.handle_input)
        
        self.text_input_layout = BoxLayout(orientation='horizontal', size_hint=(1, 1))
        ## Scrollview layout
        
        self.text_input_layout.add_widget(self.textbox)
        self.text_input_layout.add_widget(self.enter_button)
        
        self.main_layout = BoxLayout(orientation='vertical', height="50dp")
